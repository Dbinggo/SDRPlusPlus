...
ScannerModule::ScannerModule(std::string name) {
    this->name = name;
...
void ScannerModule::enable() {
    enabled = true;
}

void ScannerModule::disable() {
    enabled = false;
}

bool ScannerModule::isEnabled() {
    return enabled;
}
void ScannerModule::HPStart(){
    this->start();
}
void ScannerModule::HPStop(){
    this->stop();
}


void ScannerModule::start() {
    if (running) { return; }
    current = startFreq;
    running = true;
...
bool ScannerModule::findSignal(bool scanDir, double& bottomLimit, double& topLimit, double wfStart, double wfEnd, double wfWidth, double vfoWidth, float* data, int dataWidth) {
    bool found = false;
    double freq = current;
...
float ScannerModule::getMaxLevel(float* data, double freq, double width, int dataWidth, double wfStart, double wfWidth) {
